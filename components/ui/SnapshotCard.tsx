...
export const SnapshotCard = ({ chain, index = 0 }: SnapshotCardProps) => {
  ...
  return (
    <Link href={`/chains/${chainId}`} className="block">
      <motion.div
        ...
        className="bg-white rounded-xl border border-border p-6 hover:shadow-lg transition-all duration-200 cursor-pointer"
      >
        ...
        >
          <div className="flex justify-between items-center">
            ...
            <CopyableValue
              value={snapshots?.snapshot.block_height.toString() || ""}
              label="latest block"
            >
              <motion.span
          ...
          <div className="flex justify-between items-center">
            ...
            <CopyableValue value={chain.nodeVersion} label="node version">
              <motion.span
                whileHover={{ scale: 1.05 }}
        ...
        >
          <motion.button
            ...
            onClick={(e) => {
              e.stopPropagation();
              e.preventDefault();
              if (snapshots?.snapshot.url) {
                window.open(snapshots.snapshot.url, "_blank");
              }
            }}
