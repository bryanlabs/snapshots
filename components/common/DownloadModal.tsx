...
import { getTierBandwidth, isPremiumTier, isUltraTier, normalizeTierName } from '@/lib/utils/tier';
...
export function DownloadModal({ 
  ...
  const { user } = useAuth();
  const tier = normalizeTierName(user?.tier) || 'free';
  const isPaid = isPremiumTier(tier);
  const currentSpeed = getTierBandwidth(tier);
  const tierDisplayName = isUltraTier(tier) ? 'Ultra' : tier === 'premium' ? 'Premium' : 'Free';
  
  // Paid users get a simplified modal with their actual entitlement.
  if (isPaid) {
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{tierDisplayName} Download</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            <div className={`${isUltraTier(tier) ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800' : 'bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800'} rounded-lg p-4 border`}>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-medium ${isUltraTier(tier) ? 'text-amber-600 dark:text-amber-400' : 'text-purple-600 dark:text-purple-400'}`}>
                  {currentSpeed} Mbps
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">Estimated time:</span>
                <span className="font-medium">{calculateDownloadTime(snapshot.size, currentSpeed)}</span>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={onConfirm}
              disabled={isLoading}
              className={isUltraTier(tier) ? 'bg-amber-600 hover:bg-amber-700' : 'bg-purple-600 hover:bg-purple-700'}
            >
              {isLoading ? (
                <>
  ...
  const bandwidthInfo = {
    ...
    ultra: {
      speed: '500 Mbps',
      speedMbps: 500,
      description: 'Fastest public download pool',
      estimatedTime: calculateDownloadTime(snapshot.size, 500),
      benefits: [
        '10x faster than free downloads',
        'Highest public bandwidth allocation',
        'Custom snapshot access',
        'Priority support',
        'Ultra community access'
      ]
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          ...
          {/* Premium tier info */}
          <div className="border rounded-lg p-4 relative bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="border rounded-lg p-4 relative bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-semibold text-gray-900 dark:text-white">Ultra Tier</h4>
              <div className="text-right">
                <span className="text-lg font-bold text-amber-600 dark:text-amber-400">500 Mbps</span>
                <p className="text-xs text-gray-600 dark:text-gray-400">Estimated time: {bandwidthInfo.ultra.estimatedTime}</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
              Fastest public download pool
            </p>
            <ul className="space-y-1">
              {bandwidthInfo.ultra.benefits.map((benefit, i) => (
                <li key={i} className="flex items-start gap-2 text-sm">
                  <svg className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                  </svg>
